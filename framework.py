...
class HTTPRequest:
    ...
    def read_headers(self):
        ...
        body: str = ""
        while(True):
            recvbytesect=self.socket.recv(2048)
            recvbyte=recvbyte+recvbytesect
            if len(recvbytesect)<2048:
                break
        data = recvbyte.decode().split('\r\n')
        #get the status line's message
        self.method, self.request_target, self.http_version = data[0].split()
        c = 1
        #get head lines
        for hline in data[1:]:
            if hline !="":
                # split to get head field name and value
                Name=hline.split(": ")[0]
                Value=hline.split(": ")[1]
                ...
                break
        #this is for task3
        for subbody in data[c:]:
            if subbody!= data[-1]:
                body = body + subbody + "\r\n"
            else:
                body = body + subbody
        self.buffer = body.encode()
        # print("c"+str(self.buffer))
...
class HTTPResponse:
    ...
    def write_all(self):
        ...
        self.response = statusline.encode()
        # send status line
        self.socket.send(self.response)
        for header in self.headers:
            headline = '%s: %s\r\n' % (header.name, header.value)
            # send header lines
            self.socket.send(headline.encode())
        #send the <cr> <if>
        self.socket.send(b'\r\n')
        # send entity body
