...
class HTTPResponse:
    ...
    def write_all(self):
        ...
        self.socket.send(self.response)  # send status line
        for header in self.headers:
            headline='%s: %s\r\n'%(header.name, header.value)
            self.socket.send(headline.encode())  # send header lines
        self.socket.send(b'\r\n')
        self.socket.send(self.body) # send body
        pass
